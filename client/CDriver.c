...
const int gearUp[6]=
    {
        9500,9500,9500,9500,9500,0
    };
const int gearDown[6]=
    {
        0,3900,3900,3900,3900,3900
    };
...
float getSteer(structCarState *cs)
{
    ...
    targetAngle = (cs->angle - sqrt(abs(trackPos)) * trackPos) * 4;
    ...
    if (cs->trackPos < 1 && cs->trackPos > -1)
    {
        ...
        else
        {
            ...
            if(rxSensor>sxSensor)
            {
                ...
                targetSpeed = maxSpeed*(cSensor*sinAngle/maxSpeedDist) + 10;
            }
